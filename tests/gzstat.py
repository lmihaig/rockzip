...
class BitStream:
    ...
    def __init__(self, source_file):
        ...
        self.working_bits_read = (
            0  # Number of bits already read from self.working_bytes
        )
    ...
    def write_lz77_length_distance(self, length, distance):
        ...
        if distance > len(self.history):
            raise DecodingException(
                "Invalid length/distance (%d, %d) (history only contains %d bytes)"
                % (length, distance, len(self.history))
            )
        for i in range(length):
            self.write_byte(
                self.history[len(self.history) - distance]
            )  # Note that we do not need to increment the index we use (the sequence moves along as we add bytes)
...
def build_huffman_tree(code_table):
    ...
    for i in range(len(code_table)):
        ...
        if root is None:
            raise BuildHuffmanException(
                "Symbol 0x%02x (code %s) terminates at an internal node"
                % (i, "".join(int(b) for b in code_sequence))
            )
    return root
...
def decode_huffman(stream, output_buffer, ll_tree, dist_tree):
    ...
    while 1:
        ...
        if node is None:
            raise DecodingException(
                "Reached invalid state while decoding (code lengths were probably encoded incorrectly)"
            )
        if node.symbol == -1:
            continue
        ...
        if decode_blocks:
            if symbol >= 33 and symbol <= 127:
                ...
                print(
                    "%12s%s: 0x%02x (%s)"
                    % ("", "".join(str(b) for b in ll_path), symbol, chr(symbol))
                )
            else:
        ...
        if symbol > 256:
            ...
            if decode_blocks:
                print(
                    "%16s%d extra length bits: Offset %d. Total length %d"
                    % ("", num_extra_bits, length_offset, length)
                )
            dist_path = []
            dist_node = dist_tree
            if dist_node is None:
                raise DecodingException(
                    "Can't decode length/distance pair: No distance codes exist"
                )
            ...
            if decode_blocks:
                print(
                    "%16sDistance code %s: %d"
                    % ("", "".join(str(b) for b in dist_path), dist_symbol)
                )
            ...
            if decode_blocks:
                print(
                    "%16s%d extra distance bits: Offset %d. Total distance %d"
                    % ("", num_extra_bits, distance_offset, distance)
                )
...
def decode_uncompressed(stream, output_buffer):
    ...
    print(
        "        LEN = %d (0x%04x), NLEN = %d (0x%04x)"
        % (block_len, block_len, block_nlen, block_nlen)
    )
...
def decode_dynamic(stream, output_buffer):
    ...
    decode_print(
        "CL code lengths (0 - 18): " + " ".join(str(i) for i in CL_code_lengths)
    )
    ...
    while codes_read < num_ll_codes + num_dist_codes:
        ...
        if symbol <= 15:
            ...
            decode_print(
                "Symbol 16 (repeat count %d, repeating %d)"
...
def read_member(stream, member_number):
    ...
    header_print(
        ...
        % (compression_method, compression_methods[compression_method])
    )
    flags = stream.read_byte()
    header_print("Flags: 0x%02x" % (flags))
    mtime = stream.read_uint32_little_endian()
    header_print(
        "Modification time: 0x%08x (%s)"
        % (mtime, datetime.datetime.fromtimestamp(mtime).ctime())
    )
...
if __name__ == "__main__":
    ...
    argument_parser.add_argument(
        "--no-headers", action="store_true", help="Don't print gzip headers."
    )
    argument_parser.add_argument(
        "--no-block-stats", action="store_true", help="Don't print block stats."
    )
    argument_parser.add_argument(
        "--print-block-codes",
        action="store_true",
        help="Print code information for blocks of type 2.",
    )
    argument_parser.add_argument(
        "--decode-blocks",
        action="store_true",
        help="Print full decoding information for each block.",
    )
    ...
    try:
        stream = BitStream(
            sys.stdin.buffer
        )  # sys.stdin.buffer is a version of sys.stdin open in binary mode
        member_number = 0
